Shader "CloudRendering/RaymarchShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            uniform float2 _WeatherMap_offset; 
            uniform float _WeatherMap_scale; 
            ...
            uniform float _step_increase_rate;
            ...
            {
                float4 wm = tex2D(_WeatherMap, (pos.xz + _WeatherMap_offset) / _WeatherMap_scale);
            ...
            float sampleDensityWithoutNoise(float3 pos)
            {
                ...
                float SN = saturate(remap(1 * SA, 1 - _g_c * WMc, 1, 0, 1)) * 1;
            ...
            float3 calculateLighting(float3 pos, float density, float densityTowardsSun, float cosTheta, float blueNoise, float dist_along_ray)
            {
                ...
                float3 ret_color = attenuation * _LightColor0.rgb;
                return ret_color;
            }
            ...
            fixed4 raymarching (float3 rayOrigin, float3 rayDirection, float3 sceneColor, float maxDepth, float distFromStart, float blueNoise)
            {
                ...
                float STEP_SIZE_OUT_OF_CLOUD = _step_size_outside_cloud + distFromStart * _step_increase_rate;
                float STEP_SIZE_IN_CLOUD = _step_size_inside_cloud + distFromStart * _step_increase_rate;
                
                static const int STEPS_NEEDED_TO_EXIT_CLOUD = 3; 
                static const float CLOUD_THRESHOLD = 0.1;
                ...
                bool inCloud = false;
                int exitedCloud = RAYMARCHING_STEPS + 10;

                // rayOrigin += rayDirection * (blueNoise - 0.5) * STEP_SIZE_OUT_OF_CLOUD;
                // maxDepth += (blueNoise - 0.5) * STEP_SIZE_OUT_OF_CLOUD;
                ...
                for(int i = 0; i < RAYMARCHING_STEPS; i++)
                {
                    ...
                    if(inCloud && densitySample <= CLOUD_THRESHOLD && i - exitedCloud >= STEPS_NEEDED_TO_EXIT_CLOUD)
                    {
                        inCloud = false;
                        exitedCloud = RAYMARCHING_STEPS + 10;
                    }
                    else if(!inCloud && densitySample > CLOUD_THRESHOLD)
                    {
                        inCloud = true;
                    ...
                    // if directional light, w = 0 and xyz = the light direction
                    ...
                    float3 currentRGB = calculateLighting(pos, cloudDensity, densityTowardsSun, dot(directionToSun, rayDirection), blueNoise, distFromStart + distanceTraveled);
